...
try:
    ...
    pass
sRootArt = 'plugin.video.OTV_MEDIA/resources/lib/youtube_api/'
...
class XbmcContext(AbstractContext):
    def __init__(self, path='/', params=None, plugin_name='', plugin_id='', override=True):
        ...
        if plugin_id:
            self._addon = xbmcaddon.Addon(id='plugin.video.OTV_MEDIA')
        else:
            self._addon = xbmcaddon.Addon(id='plugin.video.OTV_MEDIA')
    ...
    def get_ui(self):
        if not self._ui:
            self._ui = XbmcContextUI(addon, weakref.proxy(self))
        return self._ui
    ...
    def localize(self, text_id, default_text=u''):
        ...
        if not result:
            try:
                result = addon.getLocalizedString(int(text_id))
                if result is not None and result:
                    result = utils.to_unicode(result)
            except ValueError:
                pass
    ...
    def send_notification(self, method, data):
        ...
        self.execute('NotifyAll(plugin.video.OTV_MEDIA,%s,%s)' % (method, data))
