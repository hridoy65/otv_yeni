...
class VideoInfo(object):
    ...
    def calculate_n(self, url):
        if not self._calculate_n:
            log_debug('`n` was not calculated for %s' % url)
            return url
    ...
    def _method_get_video_info(self, video_id):
        ...
        if playability_status.get('status', 'ok').lower() != 'ok':
            if not ((playability_status.get('desktopLegacyAgeGateReason', 0) == 1) and not get_settings().age_gate()):
                reason = None
        ...
        httpd_is_live = (get_settings()and
                         is_httpd_live(port=get_settings().httpd_port()))
        ...
        if pa_li_info and (pa_li_info != ['']) and not httpd_is_live:
            raise Exception('Proxy is not running')
    ...
    def generate_mpd(self, video_id, adaptive_fmts, duration, license_url):
        ...
        ipaddress = get_settings().httpd_listen()
        ...
        mpd_quality = get_settings().get_mpd_quality()
        hdr = get_settings().include_hdr() and 'vp9.2' in ia_capabilities
        limit_30fps = get_settings().mpd_30fps_limit()
        ...
        limit_qualities = get_settings().mpd_video_qualities()
                ...
                for i in data[key]:
                    ...
                    if 'audio' in mime:
                        ...
                        if 'vp9.2' == video_codec.lower() and ('vp9.2' not in ia_capabilities or
                                                               not get_settings().include_hdr()):
                            if not get_settings().include_hdr() and 'vp9.2' in ia_capabilities:
