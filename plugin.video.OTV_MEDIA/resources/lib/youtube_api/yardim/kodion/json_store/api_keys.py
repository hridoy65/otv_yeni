...
class APIKeyStore(JSONStore):
    ...
    def set_defaults(self):
        data = self.get_data()
        if 'keys' not in data:
            data = {'keys': {'personal': {'api_key': searchKey, 'client_id': CLIENT_ID, 'client_secret': CLIENT_SECRET}, 'developer': {}}}
        if 'personal' not in data['keys']:
            data['keys']['personal'] = {'api_key': searchKey, 'client_id': CLIENT_ID, 'client_secret': CLIENT_SECRET}
