...
YOUTUBE_API_KEY  = get_key('Xhi3_LoIzw_OizD15SyCNReMvKL27nw_OizDWRR395T5uGWpvn451I2VYc78Gy463')
...
def delete_database():
	with db:
		...
		cur.close()
	return
def rkead_url(sUrl):  # S'occupe des requetes
...
def search_channel(channel_name,page):
	my_addon = xbmcaddon.Addon()
	result_num = 50

	if page:
		req_url='https://www.googleapis.com/youtube/v3/search?q=%s&type=video&part=snippet&maxResults=%s&pageToken=%s&key=%s'%(channel_name,str(result_num),page,YOUTUBE_API_KEY)
	else:
		req_url='https://www.googleapis.com/youtube/v3/search?q=%s&type=video&part=snippet&maxResults=%s&key=%s'%(channel_name,str(result_num),YOUTUBE_API_KEY)

	logger.info("req_url : %s" % str(req_url))
	read=read_url(req_url)
#	logger.info("read : %s" % str(read))
	decoded_data=json.loads(read)
	listout=[]
	videoids=[]
	try:
...
#		desc=decoded_data['items'][x]['snippet']['description']
		videoids=decoded_data['items'][x]['id']['videoId']
		logger.info("read : %s" % str(videoids))
		video_req_url = 'https://www.googleapis.com/youtube/v3/videos?part=snippet,contentDetails,statistics&id='+videoids+'&key='+ YOUTUBE_API_KEY
		video_read = read_url(video_req_url)
		video_decoded = json.loads(video_read)
		logger.info("video_decoded : %s" % str(video_decoded ))
		...
		for x in range(0, len(sorted_data)):
				...
				logger.info("title : %s" % str(title))
				date = re.search("[0-9]{4}-[0-9]{2}-[0-9]{2}", sorted_data[x]['snippet']['publishedAt'])
				listout.append([title, video_id, thumb, desc, duration, date.group(),next_page])
	return listout
