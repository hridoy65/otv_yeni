...
def live_events(provider, context, event_type='live'):
    def _sort(x):
        return x.get_aired()
    ...
    json_data = provider.get_client(context).live_events(event_type=event_type, page_token=page_token, location=location)
    if not v3.handle_error(provider, context, json_data):
        return False
...
def process(category, provider, context):
    ...
    elif category == 'canli_yayinlar':
        return live_events(provider, context)
