...
class Provider(kodion.AbstractProvider):
    ...
    def get_client(self, context):
        ...
        if dev_id:
            ...
            if api_last_origin != 'plugin.video.OTV_MEDIA':
                context.log_debug('API key origin changed, clearing cache. |plugin.video.OTV_MEDIA|')
                access_manager.set_last_origin('plugin.video.OTV_MEDIA')
                self.get_resource_manager(context).clear()
    ...
    def _on_my_location(self, context, re_match):
        ...
        if settings.get_bool('youtube.folder.upcoming.live.show', True):
            ...
            live_events_item.set_fanart(self.get_fanart(context))
            result.append(live_events_item)
        if settings.get_bool('youtube.folder.canliyayin.show', True):
            live_events_item = DirectoryItem(context.localize(self.LOCAL_MAP['youtube.canliyayin']),
                                         context.create_uri(['special', 'canli_yayinlar']),
                                         image=context.create_resource_path('media', 'live.png'))
            live_events_item.set_fanart(self.get_fanart(context))
            result.append(live_events_item)
    ...
    def maintenance_actions(self, context, re_match):
        ...
        if action == 'clear':
            ...
            _maint_files = {'function_cache': 'cache.sqlite',
                            ...
                            'temp_files': 'special://temp/plugin.video.OTV_MEDIA/'}
    ...
    def on_root(self, context, re_match):
        ...
        if settings.get_bool('youtube.folder.canliyayin.show', True):
            live_events_item = DirectoryItem(context.localize(self.LOCAL_MAP['youtube.canliyayin']),
                                         context.create_uri(['special', 'canli_yayinlar']),
                                         image=context.create_resource_path('media', 'canliyayin.jpg'))
            live_events_item.set_fanart(self.get_fanart(context))
            result.append(live_events_item)
