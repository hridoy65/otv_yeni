...
def atvcomtr():
    ...
    liste.append( ['KLASİK DİZİLER ABC','https://www.atv.com.tr/ajax/classicseries/lazycontent?skipcount=40'] )
...
#    aResult = oParser.parse(sHtmlContent, sPattern)
    ...
    sPattern = '<div class="category-classic-item type-02"><a href="(.*?)".*?<img.*?data-src="(.*?)".*?alt="(.*?)"'
    aResult = oParser.parse(sHtmlContent, sPattern)
    ...
    if not (aResult[0] == False):
        ...
        for aEntry in aResult[1]:
            ...
            ssUrl ='https://www.atv.com.tr'+ str(aEntry[0])+'/bolumler'
          
            
            source =getHtml(ssUrl)  
            catId = re.search("categoryId:'(.*?)'",  source).group(1) 
                                                        
            sUrl ='https://www.atv.com.tr/ajax/series/infinityvideolist?pageindex=0&takecount=13&categoryid=%s&videotype=10&orderby=4&articletype=10'% (catId) 
            sTitle =malfabekodla(sTitle) 
            oOutputParameterHandler = cOutputParameterHandler()
            oOutputParameterHandler.addParameter('siteUrl', sUrl)
            oOutputParameterHandler.addParameter('sMovieTitle', str(sTitle))
...
def pageshowMovies():
        ...
        url  = getHtml(Url)
        sHtmlContent = json.loads(url)
        #logger.info('aJson>%s' % aJson)
                                                        
        sPattern = '<div class="category-fragmans-box">.*?<a href="(.*?)">.*?data-src="(.*?)".*?<span class="style-01 blue-color">(.*?)</span>'
        aResult = oParser.parse(sHtmlContent, sPattern)
        if not (aResult[0] == False):
                total = len(aResult[1])
...
def ArshowSinema():
    ...
    cata =getHtml(Url).replace("\\r","").replace("\\n","").replace("                ","").replace(" ","")  
    #s1 = json.dumps(cata)
    sHtmlContent = json.loads(cata)
    logger.info("ArshowSinema: %s" % sHtmlContent ) 
    oParser = cParser()
    #Decoupage pour cibler la partie Film ajouté    
    #sPattern = '<div class="tab-item active">(.+?)<a href="javascript:"'
                
   
    #aResult = oParser.parse(sHtmlContent, sPattern)
    #regex pour listage films sur la partie decoupée  

    sPattern = '<divclass="category-classic-item"><ahref="(.*?)"><divclass=".*?"><pictureclass="img"><imgsrc=".*?"data-src="(.*?)"class="lazyload"alt=".*?"></picture></div><h2>(.*?)</h2>'
    aResult = oParser.parse(sHtmlContent, sPattern)
