...
def start(url):
	...
	if 'ashemaletube' in url:
		...
		content = make_request(url).replace("'", '"').replace("(", "").replace(")", "")
		add_dir('[COLOR lightgreen]efukt.com	 [COLOR red]Search[/COLOR]', efukt, 1, logos + 'efukt.png', fanart)
		match = re.compile('<a  href="([^"]*)" title="([^"]*)" class="thumb" style="background-image: url"([^"]*)"').findall(content)
		...
		match = re.compile('<a class="thumb thumb-chrome video-thumb bg-dark" href="(.+?)".+?<img.+?src="(.+?)" alt=\"(.+?)\">.+?<div class="thumb-icon video-duration">(.+?)</div>', re.DOTALL).findall(content)
...
def resolve_url(url):
	...
	if 'xvideos' in url:
		...
		media_url = re.compile('<video class="player-container__no-script-video" preload="auto".+?src="(.+?)"').findall(content)[0]
		media_url = media_url.replace('\\','')
		...
		vid = re.compile('"vid":"(.+?)"').findall(content)[0]
		urll ='https://www.empflix.com/ajax/video-player/'+  vid
		conten = make_request(urll)
		conten = conten.replace('\\','')
		media_url = re.compile(' <source src="(.+?)"').findall(conten)[0]
