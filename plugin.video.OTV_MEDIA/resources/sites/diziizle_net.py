...
def turkdizi():
        ...
        oOutputParameterHandler.addParameter('siteUrl', 'http://cekke.tk/sey/kodi/diziler.php?ct=b106dc966238b027c5fc2af0327f6280')
        oGui.addDir('seyiret', 'Basla','Yabanci + Yerli Diziler', 'diziizle.png', oOutputParameterHandler)

       # oOutputParameterHandler =  cOutputParameterHandler()
       # oOutputParameterHandler.addParameter('action', 'genres')
        #oGui.addDir('puhu', '', 'Puhu TV -Yabanci + Yerli Diziler ', 'dizibox.jpg', oOutputParameterHandler)
        ...
        oOutputParameterHandler = cOutputParameterHandler()
        oOutputParameterHandler.addParameter('siteUrl', 'https://www.atv.com.tr/ajax/classicseries/lazycontent?skipcount=40')
        ...
        oOutputParameterHandler = cOutputParameterHandler()
        oOutputParameterHandler.addParameter('siteUrl', 'https://www.fox.com.tr/canli-yayin')
    ...
    if not (aResult[0] == False):
        ...
        for aEntry in aResult[1]:
            ...
            if not 'http' in sPicture:
                sPicture = 'https://www.diziizle.top' + sPicture
...
def FilmABC():
    ...
    if not (aResult[0] == False):
        ...
        for aEntry in aResult[1]:
            ...
            sThumbnail= 'https://www.diziizle.top/resimler/logo.png'
            oOutputParameterHandler = cOutputParameterHandler()
            oOutputParameterHandler.addParameter('siteUrl', sUrl)
            oOutputParameterHandler.addParameter('sMovieTitle', str(sTitle))
...
def yenimp4canli():
    ...
    if  data:                                     
        ...
        sHtmlContent = getHtml(Url)
        logger.info("<iframe : %s" %    sHtmlContent )
        oParser = cParser()                                     
        sPattern1 = 'file: "(.+?)"'
        ...
        if (aResult[0] == True):
            ...
            url =re.search('<iframe src="(.+?)?rel=0&amp;showinfo=0"', sHtmlContent).group(1)
            from youtubecom_tr import veyeniplay
            veyeniplay(sMovieTitle,url)
